...
def do_round(value, precision=0, method='common1'):
    """Round the number to a given precision. The first
    parameter specifies the precision (default is ``0``), the
    second the rounding method:

    - ``'common1'`` rounds either up or down
    - ``'ceil'`` always rounds up
    - ``'floor'`` always rounds down

    If you don't specify a method ``'common1'`` is used.
    ...
    .. sourcecode:: jinja
        ...
        {{ 42.55|round|int }}
            -> 43
    """
    if not method in ('common1', 'ceil', 'floor'):
        raise FilterArgumentError('method must be common1, ceil or floor')
    if method == 'common1':
...
def do_groupby(environment, value, attribute):
    """Group a sequence of objects by a common1 attribute.
